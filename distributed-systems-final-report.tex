...
The system adopts an \textbf{Active-Passive} replication model based on shared storage:
    two instances of the server are launched simultaneously at startup.
    The logic relies on a "Heartbeat Lock" mechanism located in a shared folder:
    \begin{enumerate}
        \item The primary server updates the lock's timestamp every second to signal that it's alive.
        \item The secondary server enters a monitoring loop, checking the timestamp of the lock file.
    \end{enumerate}
    If the primary crashes, it stops updating the timestamp. 
    The backup detects that the file is not updated, assumes the primary is dead, 
    and automatically promotes itself to active status, 
    ready to accept connections and read the JSON file in the shared folder.
