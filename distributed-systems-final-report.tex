...
\begin{itemize}
  ...
  %
  \begin{itemize}
    \item The game works as follows: multiple users can join a shared game room, where they are presented with a set of categories
    ...
    Users can join the game from anywhere in the world, as long as they have an internet connection to access the game server.
    Sessions can start at any time and consist of multiple rounds. In each round, players submit their answers and vote on others'
...
\begin{itemize}
  ...
  \begin{itemize}
    \item \textbf{Multiplayer session management}: 
      Players must be able to join a shared game room. 
      The system must support robust room creation and player management.
    \item \textbf{Core Gameplay Mechanics}:
      The game must present players with category-based challenges and an initial letter.
      At the starting of the game the players must select at max 5 categories to add to the default ones (Nomi, Cose, Città)
      ...
      Players must be able to insert their answers before a timer runs out.
      At the end of each round, the system must evaluate answers (check that the first letter is the same as the given one),
      after the players have to vote if each answer is valid or not.
    \item \textbf{REVIEW NEEDED - Collaborative Validation}: 
      The system must provide a validation phase where players can review other players' answers. 
      Players must be able to vote to accept or reject each answer via chat.
    \item \textbf{Real-time Chatting}:
      The system must provide a live chat feature for players to communicate during the game.
    \item \textbf{Scoring and Vicotry}:
      The system must include a point system. Points are awarded for validated answers, 
