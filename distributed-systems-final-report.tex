...
\begin{itemize}
  ...
  %
  \begin{itemize}
    \item TCP for game and p2p communication, UDP for discovery
  \end{itemize}
  
  \item how should \emph{in-transit data} be \textbf{represented}?
  %
  \begin{itemize}
    \item JSON in UTF-8 encoding
  \end{itemize}
  
  \item how should \emph{databases} be \textbf{queried}?
  %
  \begin{itemize}
    \item no DBMS, just file-based shared storage for server state persistence
  \end{itemize}
  
  \item how should components be \emph{authenticated}?
  %
  \begin{itemize}
    \item no authentication, just a unique username for each player
  \end{itemize}

  \item how should components be \emph{authorized}?
  %
  \begin{itemize}
    \item no authorization, any player can perform any action 
    (except for admin actions in the lobby, which are only available to the first player who joined the lobby)
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item \emph{asyncio} for asynchronous programming and concurrency
  \item \emph{socket} for network communication
  \item \emph{json} for message serialization
  \item \emph{file-based shared storage} for server state persistence
  \item \emph{Poetry} for dependency management and packaging
  \item \emph{Tkinter} for the client UI
  \item \emph{pytest} for automatic testing
\end{itemize}
