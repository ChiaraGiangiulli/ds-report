...
\begin{itemize}
    \item \textbf{Server Cluster (2 instances):} 
    Two Python processes configured in an Active-Passive architecture.
    ...
    allowing the backup node to access the latest game state and 
...
Clients are initialized with a configuration file containing the addresses of both servers.

If the connection to the current server drops, the client automatically tries to connect to the next IP in the list. 
It treats the list as a continuous loop, ensuring that if the original primary comes back online 
and the new primary fails later, the client can successfully reconnect to the original node.
